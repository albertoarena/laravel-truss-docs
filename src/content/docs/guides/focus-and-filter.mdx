...
Two numbers, never three. A filter or a focus counts against what the browser received, so a filtered diagram reads `3 of 24 tables` rather than stacking the exclusions on top. The names of excluded tables are never sent to the browser, so the footer reports how many there are and nothing more.
