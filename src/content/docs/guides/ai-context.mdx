...
`Truss::snapshot()` is built for exports, so it carries no schema diff and no doctor findings. If you are rendering the dashboard's own view rather than exporting, `Truss::payload()` returns the array it draws from, including both. See [Embedding](/guides/embedding/).
