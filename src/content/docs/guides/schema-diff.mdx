...
The baseline is the one thing Truss writes to disk. It is a structure-only JSON file (never row data) at `truss/baselines/{connection}.json` on the disk set by `diff.disk` (the default disk otherwise). It lives on disk rather than in the cache for a simple reason: once a migration has run, the previous schema cannot be rebuilt from the live database, so it must not depend on a cache that a deploy might clear.
...
- **`diff.disk`** is the filesystem disk the baseline is written to. `null` uses the application's default disk. The path is always `truss/baselines/{connection}.json`.
