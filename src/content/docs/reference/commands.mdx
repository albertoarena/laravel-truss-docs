...
Print the database structure as a table in the terminal: one row per table, with its column count and foreign-key count. It is the text counterpart to the visual dashboard, applying the same `excluded_tables` filter over the same cached snapshot, and like everything in Truss it shows structure only, never row data.

It closes with how much of the connection it printed, `24 of 32 tables on mysql` when config hid some and a plain `32 tables on mysql` when it did not, so nothing goes missing without being accounted for.
...
# The whole diagram as one file that opens with no server
php artisan truss:export --format=html --output=schema.html
```

Options: `--format` (`dbml` / `json` / `csv` / `markdown` / `mermaid` / `llm` / `html`), `--connection`, `--tables` and `--exclude` (config `excluded_tables` always wins), `--output` (a file path; stdout otherwise, and required for `html`), `--check` (exit non-zero if `--output` would change, writing nothing), `--fresh` (rebuild the snapshot first), and `--mermaid` (`html` only: `inline`, the default, or `cdn` for a small file that loads Mermaid from a URL). For AI-context output, add `--compact` (drop defaults and non-unique indexes), `--focus=<table>` with `--depth=<n>` (a table and its foreign-key neighbourhood), and `--no-annotations` (strip configured annotations). Exit codes are `0` (written or up to date), `1` (`--check` found drift), and `2` (a usage or runtime error). See the [Schema export guide](/guides/schema-export/) for the formats, the [single-file HTML export](/guides/schema-export/#the-whole-diagram-as-one-file), and the CI drift-check recipe, and [Truss as AI context](/guides/ai-context/) for the annotation, compact, and focus flags.
