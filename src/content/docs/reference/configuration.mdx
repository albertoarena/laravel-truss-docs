...
| `reveal_excluded` | `TRUSS_REVEAL_EXCLUDED` | on in local | Send excluded tables flagged and undrawn, so the viewer can show them |
...
The response does say **how many** tables the list removed, which is what lets the dashboard footer read `24 of 32 tables` instead of presenting a filtered diagram as the whole schema. A count only: the names of excluded tables are never sent. `truss:show` reports the same way in the terminal.

### `reveal_excluded`

Whether excluded tables reach the browser at all.

**Off**, they are removed server-side and the response reports only how many there were. **On**, they are sent flagged and undrawn, and the dashboard offers a **Show hidden tables** toggle that draws them muted.

It defaults to **on in local** and off everywhere else, the same shape as `enabled` and the `viewTruss` gate: local is already open to whoever is looking, so you get the toggle while you work without configuring anything, and a production dashboard has to ask for it.

This is the operator's decision and never the viewer's. **There is no query parameter for it**, so excluding a table to keep it off a shared dashboard keeps working no matter who opens the page.

Revealing never changes the schema diff or the doctor findings, which always run on the filtered set. A revealed table therefore carries no change marks and no health badges, and is drawn muted to say so.
...
It is also where `truss:export --format=html --mermaid=cdn` looks: an install that has already said where its copy of Mermaid lives gets that URL in the exported file rather than a public CDN. See [the single-file HTML export](/guides/schema-export/#the-whole-diagram-as-one-file).
...
The format `truss:export` and the `Truss` facade use when none is given, one of `dbml`, `json`, `csv`, `markdown`, `mermaid`, or `llm`. Defaults to `dbml`, a standard other tools already read. Not `html`: that format is a document and requires `--output`, which makes it a poor default for a command that otherwise writes to stdout.
